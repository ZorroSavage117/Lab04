...
public:
	...
	Pawn(const Position& pos, bool isWhite) : Piece(9, 9) { this->position = pos; if (isWhite) { this->fWhite = true; } else { fWhite = false; } }
	Pawn(int c, int r, bool isWhite) : Piece(9, 9) { this->position.set(c, r); /*Col(c); this->position.setRow(r);*/ if (isWhite) { this->fWhite = true; } else { fWhite = false; } }
